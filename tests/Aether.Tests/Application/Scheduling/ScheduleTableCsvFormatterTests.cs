...
public sealed class ScheduleTableCsvFormatterTests
{
    [Fact]
    public void Format_ShouldCreateScheduleTableGroupedByDateAndShiftKind()
    {
        ...
        var formatter = new ScheduleTableCsvFormatter();

        var csv = formatter.Format(
            scenario.Problem,
            result);

        Assert.Contains("Date,DayOfWeek,Morning,Afternoon,Night", csv);
        Assert.Contains("2026-06-01,Monday,\"Dana; Yossi\",Noa,-", csv);
        Assert.Contains("2026-06-02,Tuesday,-,-,Dana", csv);
    }
